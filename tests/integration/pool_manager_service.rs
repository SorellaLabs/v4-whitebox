...
type TestService = PoolManagerService<alloy::providers::fillers::FillProvider<alloy::providers::fillers::JoinFill<alloy::providers::Identity, alloy::providers::fillers::JoinFill<alloy::providers::fillers::GasFiller, alloy::providers::fillers::JoinFill<alloy::providers::fillers::BlobGasFiller, alloy::providers::fillers::JoinFill<alloy::providers::fillers::NonceFiller, alloy::providers::fillers::ChainIdFiller>>>>, alloy::providers::RootProvider<alloy::transports::http::Http<reqwest::Client>>>, 400>;
...
fn assert_valid_pool_data(pool: &EnhancedUniswapPool<DataLoader>) {
    ...
    assert!(matches!(pool.tick_spacing, 1 | 10 | 60 | 200 | 2000), 
            "Invalid tick spacing: {}", pool.tick_spacing);
...
mod service_initialization_tests {
    ...
    async fn test_new_service_initialization() {
        ...
        println!("✅ Service initialized with {} pools at block {}", 
                 service.pool_count(), service.current_block());
    }
    ...
    async fn test_invalid_deploy_block() {
        ...
        let provider = setup_sepolia_provider().await;
        let current_block = provider.get_block_number().await.expect("Failed to get current block");
    ...
    async fn test_controller_address_setting() {
        // Test that the controller address is set correctly in the fetch_pool_keys module
        // This is indirectly tested by successful service creation, but we can verify
        // by trying to set it again (should panic due to OnceLock)
...
mod pool_discovery_tests {
    ...
    async fn test_fetch_existing_pools() {
        ...
        for (pool_id, pool) in service.get_pools() {
            ...
            println!("Pool {:?}: {} / {}", 
                     &pool_id.to_string()[..8], pool.token0, pool.token1);
        }
    }
    ...
    async fn test_pool_data_integrity() {
        ...
        for (pool_id, pool) in service.get_pools().iter().take(5) { // Test first 5 pools
            assert_valid_pool_data(pool);
    ...
    async fn test_empty_block_range() {
        ...
        println!("Service with recent deploy block {} has {} pools", 
                 recent_block, service.pool_count());
    }
}
...
mod block_handling_tests {
    ...
    async fn test_handle_new_block() {
        ...
        let mut service = create_test_service().await.expect("Failed to create service");
        ...
        println!("✅ Processed block {}, pools: {} -> {}", 
                 next_block, initial_count, service.pool_count());
    }
    ...
    async fn test_update_to_latest_block() {
        ...
        let latest_block = service.update_to_latest_block().await
...
mod error_handling_tests {
    ...
    async fn test_network_connection_failures() {
        ...
        if let Err(PoolManagerServiceError::Provider(msg)) = result {
            assert!(msg.contains("Failed to get block number") || 
                   msg.contains("500") || 
                   msg.contains("error"));
        }
    }
    ...
    async fn test_provider_errors() {
        ...
        // Should handle gracefully - either succeed with no changes or error appropriately
        match result {
            Ok(_) => {
...
mod performance_tests {
    ...
    async fn test_large_block_range_processing() {
        ...
        assert!(duration < Duration::from_secs(30), 
                "Processing 100 blocks took too long: {:?}", duration);
    ...
    async fn test_memory_usage() {
        ...
        for (_, pool) in service.get_pools().iter().take(10) {
            ...
            assert!(pool.tick_bitmap.len() < 1000, "Pool has too many bitmap entries: {}", pool.tick_bitmap.len());
        }
